...
import type { StreamStatus } from '@/hooks/useScraperStream';
...
function StatusBadge({ status }: { status: StreamStatus }) {
  const statusConfig: Record<StreamStatus, { label: string; className: string; icon: ReactNode }> =
    {
      connecting: {
        label: 'Connecting',
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between p-2 bg-gradient-to-b from-black/80 to-transparent">
        ...
        <div className="flex items-center gap-1">
          {frameCount > 0 && (
            <span className="text-xs text-surface-400 mr-2">{frameCount} frames</span>
          )}
          {onExpand && (
            <button onClick={onExpand} className="p-1 rounded hover:bg-white/10 transition-colors">
              {isExpanded ? (
      ...
      >
        {frame ? (
          <img src={frame} alt="Scraper view" className="w-full h-full object-contain" />
        ) : (
      ...
      {showOverlay && sessionInfo?.current_url && (
        <div className="absolute bottom-0 left-0 right-0 z-10 p-2 bg-gradient-to-t from-black/80 to-transparent">
          <p className="text-xs text-surface-300 truncate">{sessionInfo.current_url}</p>
        </div>
      )}
    </div>
  );
