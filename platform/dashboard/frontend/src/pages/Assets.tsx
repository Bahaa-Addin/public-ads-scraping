...
export default function Assets() {
  ...
  const { data: assets, isLoading } = useAssets({
    ...
    has_prompt:
      filters.has_prompt === 'true' ? true : filters.has_prompt === 'false' ? false : undefined,
  ...
  const isAnyMutationLoading =
    extractMutation.isPending || generateMutation.isPending || classifyMutation.isPending;
  ...
  const sourceOptions = [{ value: '', label: 'All Sources' }, ...(filterOptions?.sources || [])];

  const ctaOptions = [{ value: '', label: 'All CTA Types' }, ...(filterOptions?.cta_types || [])];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-surface-400 mt-1">
            {isTemplate
              ? 'Preview asset browser with sample data'
              : 'Browse and manage scraped agentic ads'}
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {selectedAssets.length > 0 && (
            <span className="text-sm text-surface-400">{selectedAssets.length} selected</span>
          )}
            ...
            <EmptyState
              ...
              description={
                filters.industry || filters.source || filters.search
                  ...
                  : 'Start by scraping some agentic ads from the Scrapers page'
              }
              action={
                !filters.industry && !filters.source && !filters.search
                  ? {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {assets?.assets.map((asset) => (
            <AssetCard
              ...
              onCopyPrompt={() =>
                asset.reverse_prompt && copyPrompt(asset.reverse_prompt, asset.id)
              }
        ...
        <div className="absolute top-3 left-3">
          <div
            className={cn(
              ...
              selected ? 'bg-brand-500 border-brand-500' : 'border-white/50 bg-black/30'
            )}
          >
        ...
        {asset.reverse_prompt && (
          <div className="mt-3 pt-3 border-t border-surface-800">
            <div className="flex items-start justify-between gap-2">
              <p className="text-xs text-surface-400 line-clamp-2">{asset.reverse_prompt}</p>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onCopyPrompt();
    ...
    >
      ...
      <div
        className={cn(
          ...
          selected ? 'bg-brand-500 border-brand-500' : 'border-surface-600 bg-surface-800'
        )}
      >
        {selected && <Check className="w-3 h-3 text-white" />}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {asset.reverse_prompt && (
          <p className="text-xs text-surface-500 mt-2 line-clamp-1">{asset.reverse_prompt}</p>
        )}
      </div>
