...
Use conventional commits with a scope that matches the workstream you touched. Keep each commit focused on one logical change, and prefer referencing the active task ID from [TASKS.md](/Users/monterey/Workspace/Projs/Tasmem/agentic-ads-scraping/TASKS.md) in the body when you are working against the execution plan.
