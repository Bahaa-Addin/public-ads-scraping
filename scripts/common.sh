...
enable_node_compat_polyfills() {
  local structured_clone_polyfill="$REPO_ROOT/scripts/node-structured-clone-polyfill.cjs"

  if ! command -v node >/dev/null 2>&1; then
    return
  fi

  if node -e 'process.exit(typeof structuredClone === "function" ? 0 : 1)' >/dev/null 2>&1; then
    return
  fi

  case " ${NODE_OPTIONS-} " in
    *" --require=$structured_clone_polyfill "*)
      ;;
    *)
      export NODE_OPTIONS="${NODE_OPTIONS:+$NODE_OPTIONS }--require=$structured_clone_polyfill"
      ;;
  esac
}
